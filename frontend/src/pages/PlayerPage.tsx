...
const PlayerPage = () => {
  ...
  const handlePlayerClick = (name: string, taglinePlayer: string) => {
    if (tagLine) navigate(`/joueur/${taglinePlayer}/${name}`);
  };
  ...
  return (
    <div className="page-container">
      ...
      <p className="back-to-search">
        <a href="/recherche">← Revenir à la recherche</a>
      </p>
            ...
            return (
              <li key={index} className="match-entry">
                ...
                <p><strong>Champion :</strong>
                  {champ ? <><img src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/champion/${champ.image.full}`} alt={champ.name} width={32} className="champ-icon" />{champ.name}</> : `ID ${champId}`}
                </p>
                            ...
                            return (
                              <li key={i} className={`participant-entry ${isMainPlayer ? 'highlight' : ''}`}>
                                <div className="participant-row">
                                  {champInfo && <img src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/champion/${champInfo.image.full}`} alt={champInfo.name} width={32} />}
                                  <span
                                  ...
                                  {match.info.gameMode !== 'ARAM' && p.teamPosition && (
                                    <span className={`role-badge role-${p.teamPosition.toLowerCase()}`}>{p.teamPosition.toUpperCase()}</span>
                                  )}
                                </div>
                                ...
                                <div className="items-row">
                                  {[...Array(7)].map((_, idx) => {
                                    ...
                                    return itemId ? (
                                      <img key={idx} src={`https://ddragon.leagueoflegends.com/cdn/${version}/img/item/${itemId}.png`} alt={`Item ${idx}`} width={24} height={24} />
                                    ) : null;
                                  })}
                                </div>
